...
class WeakLabeler:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.positive_phrases = [phrase.lower() for phrase in config['positive_phrases']]
        self.negative_phrases = [phrase.lower() for phrase in config['negative_phrases']]
        
        self.positive_domains = [domain.lower() for domain in config['positive_domains']]
        self.negative_domains = [domain.lower() for domain in config['negative_domains']]
    ...
    def label_text(self, text: str, source: str, domain_type: str) -> Tuple[str, List[str]]:
...
def main():
    ...
    try:
        ...
        positive_count = len(labeled_df[labeled_df['label'] == config['positive_label']])
        negative_count = len(labeled_df[labeled_df['label'] == config['negative_label']])
        
        if positive_count < min_samples or negative_count < min_samples:
            logger.warning(f"Insufficient samples for training:")
            logger.warning(f"  {config['positive_label']}: {positive_count} (minimum: {min_samples})")
            logger.warning(f"  {config['negative_label']}: {negative_count} (minimum: {min_samples})")
