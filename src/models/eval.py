...
class ModelEvaluator:
    ...
    def evaluate_model(
        self, X_test: np.ndarray, y_test: np.ndarray, test_texts: pd.Series
    ) -> Dict[str, Any]:
        """Comprehensive model evaluation."""
    ...
    def plot_precision_recall_curve(self, output_dir: Path) -> None:
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "precision_recall_curve.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def analyze_top_features(self, output_dir: Path, n_features: int = 20) -> None:
        ...
        try:
            ...
            feature_analysis = {
                "real_news_features": [
                    {"word": w, "coefficient": c} for w, c in real_features
                ],
                "fake_news_features": [
                    {"word": w, "coefficient": c} for w, c in fake_features
                ],
            }
    ...
    def generate_report(self, output_dir: Path) -> None:
        ...
        logger.info("\nClassification Report:")
        logger.info(
            f"Real News - Precision: {class_report['0']['precision']:.4f}, "
            f"Recall: {class_report['0']['recall']:.4f}, "
            f"F1: {class_report['0']['f1-score']:.4f}"
        )
        logger.info(
            f"Fake News - Precision: {class_report['1']['precision']:.4f}, "
...
def evaluate_model(
    model_path: Path, vectorizer_path: Path, features_dir: Path, output_dir: Path
) -> Dict[str, Any]:
