...
class NicheModelTrainer:
    ...
    def _train_logistic_regression(
        ...
        logger.info(
            f"Logistic Regression - Train: {train_score:.4f}, "
    ...
    def _train_svm(
        ...
        logger.info(
            f"Linear SVM - Train: {train_score:.4f}, "
            f"Val: {val_score:.4f}, CV F1: {cv_scores.mean():.4f}±{cv_scores.std():.4f}"
        )
    ...
    def _select_best_model(self) -> None:
        ...
        logger.info(
            f"Best model for {self.niche}: {best_name} (CV F1: {best_score:.4f})"
        )
    ...
    def save_models(self, output_dir: Path) -> None:
        ...
        if self.best_model is not None:
            ...
            logger.info(
                f"Saved best model ({self.best_model_name}) to {best_model_path}"
            )

        # Save vectorizer
        if self.vectorizer is not None:
            vectorizer_path = (
                output_dir.parent / "vectorizers" / f"vectorizer_{self.niche}.joblib"
            )
...
def train_niche_model(niche: str) -> Dict[str, Any]:
    ...
    X_train, y_train = vectorizer.fit_transform(
        train_df["text_processed"], train_df["label"]
    )
