...
def load_all_niche_data(niches: List[str]) -> pd.DataFrame:
    ...
    for niche in niches:
        ...
        df["label_binary"] = df["label"].map(
            {niche_config["positive_label"]: 1, niche_config["negative_label"]: 0}
        )
    ...
    logger.info(
        f"Combined dataset: {len(combined_df)} samples from {len(niches)} niches"
    )
...
def split_unified_data(df: pd.DataFrame) -> tuple:
    ...
    logger.info("Unified data splits:")
...
class UnifiedModelTrainer:
    ...
    def _train_logistic_regression(
        ...
        logger.info(
            f"Unified Logistic Regression - Train: {train_score:.4f}, "
    ...
    def _train_svm(
        ...
        logger.info(
            f"Unified Linear SVM - Train: {train_score:.4f}, "
            f"Val: {val_score:.4f}, CV F1: {cv_scores.mean():.4f}±{cv_scores.std():.4f}"
        )
    ...
    def save_models(self, output_dir: Path) -> None:
        ...
        if self.best_model is not None:
            ...
            logger.info(
                f"Saved best unified model ({self.best_model_name}) to {best_model_path}"
            )
...
def train_unified_model(niches: List[str]) -> Dict[str, Any]:
    ...
    X_train, y_train = vectorizer.fit_transform(
        train_df["text_processed"], train_df["label_binary"]
    )
    X_val, y_val = vectorizer.transform(
        val_df["text_processed"], val_df["label_binary"]
    )

    logger.info(
        f"Unified feature matrix shapes - Train: {X_train.shape}, Val: {X_val.shape}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train unified model for multiple niches"
    )
    parser.add_argument(
        "--niches", required=True, help="Comma-separated list of niches to include"
    )
