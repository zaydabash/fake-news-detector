...
def load_predictor() -> FakeNewsPredictor:
    ...
    vectorizer_path = (
        project_root / "artifacts" / "vectorizers" / "tfidf_vectorizer.joblib"
    )
...
def predict_text(
    ...
    try:
        ...
        console.print(
            f"[italic]\"{text[:100]}{'...' if len(text) > 100 else ''}\"[/italic]\n"
        )
...
def predict_file(
    file_path: str = typer.Argument(
        ..., help="Path to CSV file with texts to classify"
    ),
    text_column: str = typer.Option("text", help="Name of the column containing texts"),
    output_file: Optional[str] = typer.Option(
        None, help="Output file path (default: predictions.csv)"
    ),
):
    ...
    try:
        ...
        if text_column not in df.columns:
            console.print(
                f"[red]Error: Column '{text_column}' not found in CSV file.[/red]"
            )
